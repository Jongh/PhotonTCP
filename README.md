...
- **`colorfmt="luminance"` 텍스처가 데스크톱 GL 백엔드에서 수용됩니다.** `Texture.create` +
  ...
  `Image` 위젯에 올립니다. (안드로이드 GLES는 M12 시점엔 미검증이었고, **M13 실기에서 수용이 확인**됐습니다 — 아래 「실기 실측」 참조.)
...
#### 실기 실측 (M13, 2026-09-09) — **빛만으로 통신 성립**

안드로이드 기기 **2대**(Galaxy S26 ↔ Galaxy Tab S9 FE+)를 약 20 cm 거리에서 화면이 서로를
향하도록 놓고 돌려, **양쪽 모두 `PASS`** 를 받았습니다 — 핸드셰이크 → 메시지 2/2 일치 →
양쪽 정상 종료가 전부 빛 위로 오갔습니다.

```
receiver (Tab S9 FE+) : PASS | established=True | MATCH (2/2) | closed=True | wall=16.2s
sender   (S26)        : PASS | established=True | MATCH (2/2) | closed=True | wall=14.0s
```

조건: 양쪽 `front` 카메라 · `scale=8` · `hold=0.8` · 실내 형광등 · 약 20 cm. 상세와 튜닝
가이드는 [`docs/v1.0-signoff.md`](docs/v1.0-signoff.md)의 "사인오프 실측 기록"에 있습니다.

**성공의 관건은 카메라 정렬 하나였습니다** — 실패한 시도들의 공통점은 상대 QR이 카메라
프레임에서 너무 작거나 비스듬한 것이었습니다.

#### 현재 한계 (정직하게)

- **v1.0은 아직 주장하지 않습니다.** 사인오프 게이트는 두 조건인데(**(a)** 셀프체크 수신율
  ≥ 80%, **(b)** 2-머신 왕복), 위에서 통과한 것은 **(b)** 입니다. **(a)** 는 한 기기가 자기
  화면을 자기 카메라로 봐야 하는데, 개발 PC에는 카메라가 없고 폰·태블릿은 화면과 전면
  카메라가 같은 방향이라 자기 화면을 직접 볼 수 없어 **수행하지 못했습니다**.
- **camera4kivy는 아카이브(유지보수 중단) 상태**입니다. **분석 버퍼의 재사용 여부**는 여전히
  미검증이라 `KivyCamera`는 기본으로 제출 시점에 버퍼를 복사합니다
  (`copy_on_submit=True` — 콜백 스레드의 memcpy 한 장). 픽셀 콜백은 **경로가 동작함이 확인**됐습니다(위 실측이 그 경로를 통과했습니다). 다만
  **콜백의 정확한 인자 목록은 기기에서 기록되지 않았습니다** — 코드가 `*args`로 흡수하고 있어
  동작에는 지장이 없으나 미측정 항목으로 남습니다.
- **`flip_*` 플래그는 디코드에 영향이 없습니다.** 이전 판의 이 절은 *"QR은 상하/좌우가
  뒤집히면 디코드되지 않으므로 이 플래그부터 뒤집어 보라"* 고 적었으나 **사실이 아닙니다** —
  `cv2.QRCodeDetector`는 90/180/270° 회전과 **좌우·상하 반전(거울상)을 모두 디코드합니다**
  (M13 측정: 7개 변형 전부 성공). 디코드가 0이면 **정렬·거리·조명을 먼저** 보십시오.
- `luminance` 텍스처는 **안드로이드 GLES에서도 수용**됩니다(실기 확인 — Xclipse 960/ANGLE,
  Mali-G68 양쪽).
- APK에 opencv를 넣을지는 `buildozer.spec`에 사유와 함께 명시돼 있습니다. 레시피가 실패해
  opencv를 빼면 앱은 뜨지만 **QR을 한 장도 디코드하지 못해** 광학 링크가 성립하지 않습니다
  (우회 절차와 그 의미는 `docs/mobile-build.md` 5-3 참조). 대체 디코더 백엔드(ZXing 등)는
  opencv가 실기에서 정상 동작함이 확인됐으므로 **당장은 정당화되지 않습니다**.
- **고밀도 화면에서 설정 행이 잘립니다** — 위젯 높이가 픽셀로 고정돼 있어 S26(3배 밀도)에서는
  글자가 잘려 보입니다(태블릿에서는 정상). 기능에는 지장이 없습니다.
